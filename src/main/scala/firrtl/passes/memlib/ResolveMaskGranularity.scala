...
object AnalysisUtils {
  ...
  def getOrigin(connects: Connects, s: String): Expression =
    getOrigin(connects)(WRef(s, UnknownType, UnknownLabel, ExpKind, UNKNOWNGENDER))
  def getOrigin(connects: Connects)(e: Expression): Expression = e match { 
    case Mux(cond, tv, fv, _, _) =>
    ...
    case DoPrim(PrimOps.Or, args, consts, tpe, lbl) if args exists (weq(_, one)) => one
    case DoPrim(PrimOps.And, args, consts, tpe, lbl) if args exists (weq(_, zero)) => zero
    case DoPrim(PrimOps.Bits, args, Seq(msb, lsb), tpe, lbl) =>  
    ...
    case DoPrim((PrimOps.AsUInt | PrimOps.AsSInt | PrimOps.AsClock), args, _, _ ,_) => 
      getOrigin(connects)(args.head)
    case ValidIf(cond, value, ClockType, lbl) => getOrigin(connects)(value)
